...
public class BTreeFile extends IndexFile implements GlobalConst {
	...
	private void _destroyFile(PageID pageno) throws IOException,
		...
		if (sortedPage.getType() == NodeType.INDEX) {
			...
			PageID childId;
			KeyDataEntry entry;
			for (entry = indexPage.getFirst(genid); entry != null; entry = indexPage
					.getNext(genid)) {
	 ...
	 * @param genid
	 *            the genid of the record. Input parameter.
	 ...
	 */
	public void insert(KeyClass key, TID genid) throws KeyTooLongException,
...

	{
		...
		if (key instanceof StringKey) {
			if (headerPage.get_keyType() != AttrType.attrString) {
				// KeyClass globalKey=AttrType.attrString;
				short globalKey = 0;
				headerPage.set_keyType(globalKey);
			}
			// if ( headerPage.get_keyType() != AttrType.attrString ) {
		...
		if (trace != null) {
			trace.writeBytes("INSERT " + genid.pageNo + " " + genid.slotNo
					+ " " + key + lineSep);
	 ...
	 */
	private KeyDataEntry _insert(KeyClass key, TID genid, PageID currentPageId)
...

	{
		...
		if (currentPage.getType() == NodeType.INDEX) {
			...
			for (tmpEntry = currentIndexPage.getFirst(delGenid); tmpEntry != null; tmpEntry = currentIndexPage
					.getFirst(delGenid)) {
			...
			TID firstGenid = new TID();
			KeyDataEntry undoEntry = null;
			for (tmpEntry = newIndexPage.getFirst(firstGenid); (currentIndexPage
			...
			if (currentIndexPage.available_space() < newIndexPage
				...
				currentIndexPage
						.deleteSortedRecord(new TID(currentIndexPage
			...
			if (BT.keyCompare(upEntry.key, tmpEntry.key) >= 0) {
				...
				currentIndexPage.deleteSortedRecord(new TID(currentIndexPage
						.getCurPage(), i));
			...
			for (tmpEntry = currentLeafPage.getFirst(firstGenid); tmpEntry != null; tmpEntry = currentLeafPage
					.getFirst(firstGenid)) {
			...
			KeyDataEntry undoEntry = null;
			for (tmpEntry = newLeafPage.getFirst(firstGenid); newLeafPage
			...
			if (BT.keyCompare(key, undoEntry.key) < 0) {
				...
				if (currentLeafPage.available_space() < newLeafPage
					...
					currentLeafPage.deleteSortedRecord(new TID(currentLeafPage
							.getCurPage(),
	...
	/**
	 * delete leaf entry given its <key, genid> pair. `genid' is IN the data
	 * entry; it is not the id of the data entry)
	 * 
	 * @param key
	 *            the key in pair <key, genid>. Input Parameter.
	 * @param genid
	 *            the genid in pair <key, genid>. Input Parameter.
	 ...
	 * 
	 */
	public boolean Delete(KeyClass key, TID genid)
	...
	/*
	 * findRunStart. Status BTreeFile::findRunStart (const void lo_key, TID
	 * *pstartgenid)
	 * 
	 * find left-most occurrence of `lo_key', going all the way left if lo_key
	 * is null.
	 * 
	 * Starting record returned in *pstartgenid, on page *pppage, which is
	 * pinned.
	 * 
	...
	BTLeafPage findRunStart(KeyClass lo_key, TID startgenid)
			throws IOException, IteratorException, KeyNotMatchException,
	...
	/*
	 * Status BTreeFile::NaiveDelete (const void *key, const TID genid)
	 * 
	 ...
	 * find the one containing <key,genid>, which we then delete via
	 * BTLeafPage::delUserGenid.
	 */

	private boolean NaiveDelete(KeyClass key, TID genid)
		 ...
		 * should have got true back from delUserGenid above. Apparently the
		 * specified <key,genid> data entry does not exist.
		 */
	...
	/*
	 * Status BTreeFile::FullDelete (const void *key, const TID genid)
	 * 
	 * Remove specified data entry (<key, genid>) from an index.
	 * 
	 ...
	 * (just a few) pages, if necesary, to find the one containing <key,genid>,
	 * which we then delete via BTLeafPage::delUserGenid.
	 * 
	 * @return false if no such record; true if succees
	 */

	private boolean FullDelete(KeyClass key, TID genid)
	 ...
	 * @throws IOException
	 */
	private KeyClass _Delete(KeyClass key, TID genid, PageID currentPageId,
		...
		if (sortPage.getType() == NodeType.LEAF) {
			...
			TID delGenid;
			// for all records with key equal to 'key', delete it if its genid =
			// 'genid'
			while ((tmpEntry != null)
				...
				if (leafPage.delEntry(new KeyDataEntry(key, genid))) {
					// successfully found <key, genid> on this page and deleted
					// it.
					...
					if ((4 + leafPage.available_space()) <= ((MAX_SPACE - THFPage.DPFIXED) / 2)) {
						...
						if (siblingPage.redistribute(leafPage, parentPage,
							...
							return null;
																	 ...
																	 * 2*sizeof(slot
																	 * )
																	 */) >= ((MAX_SPACE - THFPage.DPFIXED) - leafPage
								.available_space())) {
							...
							TID firstGenid = new TID(), insertGenid;
							for (tmpEntry = rightChild.getFirst(firstGenid); tmpEntry != null; tmpEntry = rightChild
									.getFirst(firstGenid)) {
		...
		if (sortPage.getType() == NodeType.INDEX) {
			...
			KeyClass oldChildKey = _Delete(key, genid, childPageId,
					currentPageId);
			...
			PageID dummyPageId;
			KeyClass deletedKey = key;
			TID curGenid = indexPage.deleteKey(oldChildKey);
			...
			if ((4 /* sizeof slot */+ indexPage.available_space()) <= ((MAX_SPACE - THFPage.DPFIXED) / 2)) {
				...
				if (direction == 1) {
					pushKeySize = BT.getKeyLength(parentPage
							.findKey(siblingPage.getFirst(new TID()).key));
				} else if (direction == -1) {
					pushKeySize = BT.getKeyLength(parentPage.findKey(indexPage
							.getFirst(new TID()).key));
				}
				...
				if (siblingPage.redistribute(indexPage, parentPage, direction,
					...
					return null;
				} else if (siblingPage.available_space() + 4 /* slot size */>= ((MAX_SPACE - THFPage.DPFIXED)
					...
					// move all entries from rightChild to leftChild
					for (KeyDataEntry tmpEntry = rightChild
							.getFirst(firstGenid); tmpEntry != null; tmpEntry = rightChild
							.getFirst(firstGenid)) {
	...
	void trace_children(PageID id) throws IOException, IteratorException,
		...
		if (trace != null) {
			...
			if (sortedPage.getType() == NodeType.INDEX) {
				...
				for (entry = indexPage.getFirst(metaGenid); entry != null; entry = indexPage
						.getNext(metaGenid)) {
				...
				for (entry = leafPage.getFirst(metaGenid); entry != null; entry = leafPage
						.getNext(metaGenid)) {
