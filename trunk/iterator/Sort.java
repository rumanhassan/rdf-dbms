package iterator;

import java.io.*; 
import global.*;
import heap.Tuple;
import bufmgr.*;
import diskmgr.*;
import tripleheap.*;
import index.*;
import chainexception.*;

/**
 * The Sort class sorts a file. All necessary information are passed as 
 * arguments to the constructor. After the constructor call, the user can
 * repeatly call <code>get_next()</code> to get triples in sorted order.
 * After the sorting is done, the user should call <code>close()</code>
 * to clean up.
 */
public class Sort extends TripleIterator implements GlobalConst
{
  private static final int ARBIT_RUNS = 10;
  
  private AttrType[]  _in;         
  private short       n_cols;
  private short[]     str_lens;
  private TripleIterator    _am;
  private int         _sort_fld;
  private TripleOrder  order;
  private int         _n_pages;
  private byte[][]    bufs;
  private boolean     first_time;
  private int         Nruns;
  private int         max_elems_in_heap;
  private int         sortFldLen;
  private int         triple_size;
  
  private pnodeSplayPQ Q;
  private TripleHeapFile[]   temp_files; 
  private int          n_tempfiles;
  private Triple        output_triple;  
  private int[]        n_triples;
  private int          n_runs;
  private Triple        op_buf;
  private OBuf         o_buf;
  private SpoofIbuf[]  i_buf;
  private PageID[]     bufs_pids;
  private boolean useBM = true; // flag for whether to use buffer manager
  
  
  /** 
   * Class constructor, take information about the triples, and set up 
   * the sorting
   * @param in array containing attribute types of the relation
   * @param len_in number of columns in the relation
   * @param str_sizes array of sizes of string attributes
   * @param am an iterator for accessing the triples
   * @param sort_fld the field number of the field to sort on
   * @param sort_order the sorting order (ASCENDING, DESCENDING)
   * @param sort_field_len the length of the sort field
   * @param n_pages amount of memory (in pages) available for sorting
   * @exception IOException from lower layers
   * @exception SortException something went wrong in the lower layer. 
   */
  public Sort(AttrType[] in,         
	      short      len_in,             
	      short[]    str_sizes,
	      TripleIterator   am,                 
	      int        sort_fld,          
	      TripleOrder sort_order,     
	      int        sort_fld_len,  
	      int        n_pages      
	      ) throws IOException, SortException
  {
    _in = new AttrType[len_in];
    n_cols = len_in;
    int n_strs = 0;

    for (int i=0; i<len_in; i++) {
      _in[i] = new AttrType(in[i].attrType);
      if (in[i].attrType == AttrType.attrString) {
	n_strs ++;
      } 
    }
    
    str_lens = new short[n_strs];
    
    n_strs = 0;
    for (int i=0; i<len_in; i++) {
      if (_in[i].attrType == AttrType.attrString) {
	str_lens[n_strs] = str_sizes[n_strs];
	n_strs ++;
      }
    }
    
    Triple t = new Triple(); // need Triple.java
    /*try {
      t.setHdr(len_in, _in, str_sizes);
    }
    catch (Exception e) {
      throw new SortException(e, "Sort.java: t.setHdr() failed");
    }*/
    triple_size = t.size();
    
    _am = am;
    _sort_fld = sort_fld;
    order = sort_order;
    _n_pages = n_pages;
    
    // this may need change, bufs ???  need io_bufs.java
    //    bufs = get_buffer_pages(_n_pages, bufs_pids, bufs);
    bufs_pids = new PageID[_n_pages];
    bufs = new byte[_n_pages][];

    if (useBM) {
      try {
	get_buffer_pages(_n_pages, bufs_pids, bufs);
      }
      catch (Exception e) {
	throw new SortException(e, "Sort.java: BUFmgr error");
      }
    }
    else {
      for (int k=0; k<_n_pages; k++) bufs[k] = new byte[MAX_SPACE];
    }
    
    first_time = true;
    
    // as a heuristic, we set the number of runs to an arbitrary value
    // of ARBIT_RUNS
    temp_files = new TripleHeapFile[ARBIT_RUNS];
    n_tempfiles = ARBIT_RUNS;
    n_triples = new int[ARBIT_RUNS]; 
    n_runs = ARBIT_RUNS;

    try {
      temp_files[0] = new TripleHeapFile(null);
    }
    catch (Exception e) {
      throw new SortException(e, "Sort.java: Heapfile error");
    }
    
    /**
     *O_buf::Put takes triples and stores them on the buffer pages that
     *were passed to O_buf::init.  O_buf::flush inserts them enmass into
     *a temporary HeapFile.
     */

    o_buf = new OBuf();
    
    o_buf.init(bufs, _n_pages, triple_size, temp_files[0], false);
    //    output_triple = null;
    
    max_elems_in_heap = 200;
    sortFldLen = sort_fld_len;
    
    Q = new pnodeSplayPQ(sort_fld, in[sort_fld - 1], order);

    op_buf = new Triple();   // need Triple.java
   /* try {
      op_buf.setHdr(n_cols, _in, str_lens);
    }
    catch (Exception e) {
      throw new SortException(e, "Sort.java: op_buf.setHdr() failed");
    }*/
  }
  
  
  
  /**
   * Returns the next triple in sorted order.
   * Note: You need to copy out the content of the triple, otherwise it
   *       will be overwritten by the next <code>get_next()</code> call.
   * @return the next triple, null if all triples exhausted
   * @exception IOException from lower layers
   * @exception SortException something went wrong in the lower layer. 
   * @exception JoinsException from <code>generate_runs()</code>.
   * @exception UnknowAttrType attribute type unknown
   * @exception LowMemException memory low exception
   * @exception Exception other exceptions
   */
  public Triple get_next() 
    throws IOException, 
	   SortException, 
	   UnknowAttrType,
	   LowMemException, 
	   JoinsException,
	   Exception
  {
    if (first_time) {
      // first get_next call to the sort routine
      first_time = false;
      
      // generate runs
      Nruns = generate_runs(max_elems_in_heap, _in[_sort_fld-1], sortFldLen); /* _in in array containing attribute types of the relation*/
      //      System.out.println("Generated " + Nruns + " runs");
      
      // setup state to perform merge of runs. 
      // Open input buffers for all the input file
      setup_for_merge(triple_size, Nruns);
    }
    
    if (Q.empty()) {  
      // no more triples availble
      return null;
    }
    
    output_triple = delete_min();
    if (output_triple != null){
      op_buf.tripleCopy(output_triple);
      return op_buf; 
    }
    else 
      return null; 
  }

  
  /**
   * Set up for merging the runs.
   * Open an input buffer for each run, and insert the first element (min)
   * from each run into a heap. <code>delete_min() </code> will then get 
   * the minimum of all runs.
   * @param triple_size size (in bytes) of each triple
   * @param n_R_runs number of runs
   * @exception IOException from lower layers
   * @exception LowMemException there is not enough memory to 
   *                 sort in two passes (a subclass of SortException).
   * @exception SortException something went wrong in the lower layer. 
   * @exception Exception other exceptions
   */
  
  
  private void setup_for_merge(int triple_size, int n_R_runs)
    throws IOException, 
	   LowMemException, 
	   SortException,
	   Exception
  {
    // don't know what will happen if n_R_runs > _n_pages
    if (n_R_runs > _n_pages) 
      throw new LowMemException("Sort.java: Not enough memory to sort in two passes."); 

    int i;
    pnode cur_node;  // need pq_defs.java
    
    i_buf = new SpoofIbuf[n_R_runs];   // need io_bufs.java
    for (int j=0; j<n_R_runs; j++) i_buf[j] = new SpoofIbuf();
    
    // construct the lists, ignore TEST for now
    // this is a patch, I am not sure whether it works well -- bingjie 4/20/98
    
    for (i=0; i<n_R_runs; i++) {
      byte[][] apage = new byte[1][];
      apage[0] = bufs[i];

      // need iobufs.java
      i_buf[i].init(temp_files[i], apage, 1, triple_size, n_triples[i]);

      cur_node = new pnode();
      cur_node.run_num = i;
      
      // may need change depending on whether Get() returns the original
      // or make a copy of the triple, need io_bufs.java ???
      Triple temp_triple = new Triple();

      
      temp_triple =i_buf[i].Get(temp_triple);  // need io_bufs.java
            
      if (temp_triple != null) {
	/*
	System.out.print("Get triple from run " + i);
	temp_triple.print(_in);
	*/
	cur_node.triple = temp_triple; // no copy needed
	try {
	  Q.enq(cur_node);
	}
	catch (UnknowAttrType e) {
	  throw new SortException(e, "Sort.java: UnknowAttrType caught from Q.enq()");
	}
	catch (TripleUtilsException e) {
	  throw new SortException(e, "Sort.java: TripleUtilsException caught from Q.enq()");
	}

      }
    }
    return; 
  }
  
  /**
   * Generate sorted runs.
   * Using heap sort.
   * @param  max_elems    maximum number of elements in heap
   * @param  sortFldType  attribute type of the sort field
   * @param  sortFldLen   length of the sort field
   * @return number of runs generated
   * @exception IOException from lower layers
   * @exception SortException something went wrong in the lower layer. 
   * @exception JoinsException from <code>Iterator.get_next()</code>
   */
  private int generate_runs(int max_elems, AttrType sortFldType, int sortFldLen) 
    throws IOException, 
	   SortException, 
	   UnknowAttrType,
	   TripleUtilsException,
	   JoinsException,
	   Exception
  {
    Triple triple; 
    pnode cur_node;
    pnodeSplayPQ Q1 = new pnodeSplayPQ(_sort_fld, sortFldType, order); 
 /*/**
 * Implements a sorted binary tree (extends class pnodePQ).
 * Implements the <code>enq</code> and the <code>deq</code> functions.
 */
    pnodeSplayPQ Q2 = new pnodeSplayPQ(_sort_fld, sortFldType, order);
    pnodeSplayPQ pcurr_Q = Q1;
    pnodeSplayPQ pother_Q = Q2; 
    Triple lastElem = new Triple();  // need triple.java
    /*try {
      lastElem.setHdr(n_cols, _in, str_lens);
    }
    catch (Exception e) {
      throw new SortException(e, "Sort.java: setHdr() failed");
    }*/
    
    int run_num = 0;  // keeps track of the number of runs

    // number of elements in Q
    //    int nelems_Q1 = 0;
    //    int nelems_Q2 = 0;
    int p_elems_curr_Q = 0;
    int p_elems_other_Q = 0;
    
    int comp_res;
    
    // set the lastElem to be the minimum value for the sort field
    if(order.tripleOrder == TripleOrder.Ascending) {
      try {
	MIN_VAL(lastElem, sortFldType);
      } catch (UnknowAttrType e) {
	throw new SortException(e, "Sort.java: UnknowAttrType caught from MIN_VAL()");
      } catch (Exception e) {
	throw new SortException(e, "MIN_VAL failed");
      } 
    }
    else {
      try {
	MAX_VAL(lastElem, sortFldType);
      } catch (UnknowAttrType e) {
	throw new SortException(e, "Sort.java: UnknowAttrType caught from MAX_VAL()");
      } catch (Exception e) {
	throw new SortException(e, "MIN_VAL failed");
      } 
    }
    
    // maintain a fixed maximum number of elements in the heap
    while ((p_elems_curr_Q + p_elems_other_Q) < max_elems) {
      try {
	triple = _am.get_next();  // according to Iterator.java goes to filescan.java which calls TScan
      } catch (Exception e) {
	e.printStackTrace(); 
	throw new SortException(e, "Sort.java: get_next() failed");
      } 
      
      if (triple == null) {
	break;
      }
      cur_node = new pnode();
      cur_node.triple = new Triple(triple); // triple copy needed --  Bingjie 4/29/98 

      pcurr_Q.enq(cur_node);
      p_elems_curr_Q ++;
    }
    
    // now the queue is full, starting writing to file while keep trying
    // to add new triples to the queue. The ones that does not fit are put
    // on the other queue temperarily
    while (true) {
      cur_node = pcurr_Q.deq();
      if (cur_node == null) break; 
      p_elems_curr_Q --;
      
      comp_res = TripleUtils.CompareTripleWithValue(sortFldType, cur_node.triple, _sort_fld, lastElem);  // need triple_utils.java
      
      if ((comp_res < 0 && order.tripleOrder == TripleOrder.Ascending) || (comp_res > 0 && order.tripleOrder == TripleOrder.Descending)) {
	// doesn't fit in current run, put into the other queue
	try {
	  pother_Q.enq(cur_node);
	}
	catch (UnknowAttrType e) {
	  throw new SortException(e, "Sort.java: UnknowAttrType caught from Q.enq()");
	}
	p_elems_other_Q ++;
      }
      else {
	// set lastElem to have the value of the current triple,
	// need triple_utils.java
	TripleUtils.SetValue(lastElem, cur_node.triple, _sort_fld, sortFldType);
	// write triple to output file, need io_bufs.java, type cast???
	//	System.out.println("Putting triple into run " + (run_num + 1)); 
	//	cur_node.triple.print(_in);
	
	o_buf.Put(cur_node.triple);
      }
      
      // check whether the other queue is full
      if (p_elems_other_Q == max_elems) {
	// close current run and start next run
	n_triples[run_num] = (int) o_buf.flush();  // need io_bufs.java
	run_num ++;

	// check to see whether need to expand the array
	if (run_num == n_tempfiles) {
	  TripleHeapFile[] temp1 = new TripleHeapFile[2*n_tempfiles];
	  for (int i=0; i<n_tempfiles; i++) {
	    temp1[i] = temp_files[i];
	  }
	  temp_files = temp1; 
	  n_tempfiles *= 2; 

	  int[] temp2 = new int[2*n_runs];
	  for(int j=0; j<n_runs; j++) {
	    temp2[j] = n_triples[j];
	  }
	  n_triples = temp2;
	  n_runs *=2; 
	}
	
	try {
	    temp_files[run_num] = new TripleHeapFile(null);
	}
	catch (Exception e) {
	  throw new SortException(e, "Sort.java: create Heapfile failed");
	}
	
	// need io_bufs.java
	o_buf.init(bufs, _n_pages, triple_size, temp_files[run_num], false);
	
	// set the last Elem to be the minimum value for the sort field
	if(order.tripleOrder == TripleOrder.Ascending) {
	  try {
	    MIN_VAL(lastElem, sortFldType);
	  } catch (UnknowAttrType e) {
	    throw new SortException(e, "Sort.java: UnknowAttrType caught from MIN_VAL()");
	  } catch (Exception e) {
	    throw new SortException(e, "MIN_VAL failed");
	  } 
	}
	else {
	  try {
	    MAX_VAL(lastElem, sortFldType);
	  } catch (UnknowAttrType e) {
	    throw new SortException(e, "Sort.java: UnknowAttrType caught from MAX_VAL()");
	  } catch (Exception e) {
	    throw new SortException(e, "MIN_VAL failed");
	  } 
	}
    
	// switch the current heap and the other heap
	pnodeSplayPQ tempQ = pcurr_Q;
	pcurr_Q = pother_Q;
	pother_Q = tempQ;
	int tempelems = p_elems_curr_Q;
	p_elems_curr_Q = p_elems_other_Q;
	p_elems_other_Q = tempelems;
      }
      
      // now check whether the current queue is empty
      else if (p_elems_curr_Q == 0) {
	while ((p_elems_curr_Q + p_elems_other_Q) < max_elems) {
	  try {
	    triple = _am.get_next();  // according to Iterator.java
	  } catch (Exception e) {
	    throw new SortException(e, "get_next() failed");
	  } 
	  
	  if (triple == null) {
	    break;
	  }
	  cur_node = new pnode();
	  cur_node.triple = new Triple(triple); // triple copy needed --  Bingjie 4/29/98 

	  try {
	    pcurr_Q.enq(cur_node);
	  }
	  catch (UnknowAttrType e) {
	    throw new SortException(e, "Sort.java: UnknowAttrType caught from Q.enq()");
	  }
	  p_elems_curr_Q ++;
	}
      }
      
      // Check if we are done
      if (p_elems_curr_Q == 0) {
	// current queue empty despite our attemps to fill in
	// indicating no more triples from input
	if (p_elems_other_Q == 0) {
	  // other queue is also empty, no more triples to write out, done
	  break; // of the while(true) loop
	}
	else {
	  // generate one more run for all triples in the other queue
	  // close current run and start next run
	  n_triples[run_num] = (int) o_buf.flush();  // need io_bufs.java
	  run_num ++;
	  
	  // check to see whether need to expand the array
	  if (run_num == n_tempfiles) {
	    TripleHeapFile[] temp1 = new TripleHeapFile[2*n_tempfiles];
	    for (int i=0; i<n_tempfiles; i++) {
	      temp1[i] = temp_files[i];
	    }
	    temp_files = temp1; 
	    n_tempfiles *= 2; 
	    
	    int[] temp2 = new int[2*n_runs];
	    for(int j=0; j<n_runs; j++) {
	      temp2[j] = n_triples[j];
	    }
	    n_triples = temp2;
	    n_runs *=2; 
	  }

	  try {
	    temp_files[run_num] = new TripleHeapFile(null); 
	  }
	  catch (Exception e) {
	    throw new SortException(e, "Sort.java: create Heapfile failed");
	  }
	  
	  // need io_bufs.java
	  o_buf.init(bufs, _n_pages, triple_size, temp_files[run_num], false);
	  
	  // set the last Elem to be the minimum value for the sort field
	  if(order.tripleOrder == TripleOrder.Ascending) {
	    try {
	      MIN_VAL(lastElem, sortFldType);
	    } catch (UnknowAttrType e) {
	      throw new SortException(e, "Sort.java: UnknowAttrType caught from MIN_VAL()");
	    } catch (Exception e) {
	      throw new SortException(e, "MIN_VAL failed");
	    } 
	  }
	  else {
	    try {
	      MAX_VAL(lastElem, sortFldType);
	    } catch (UnknowAttrType e) {
	      throw new SortException(e, "Sort.java: UnknowAttrType caught from MAX_VAL()");
	    } catch (Exception e) {
	      throw new SortException(e, "MIN_VAL failed");
	    } 
	  }
	
	  // switch the current heap and the other heap
	  pnodeSplayPQ tempQ = pcurr_Q;
	  pcurr_Q = pother_Q;
	  pother_Q = tempQ;
	  int tempelems = p_elems_curr_Q;
	  p_elems_curr_Q = p_elems_other_Q;
	  p_elems_other_Q = tempelems;
	}
      } // end of if (p_elems_curr_Q == 0)
    } // end of while (true)

    // close the last run
    n_triples[run_num] = (int) o_buf.flush();
    run_num ++;
    
    return run_num; 
  }
  
  /**
   * Remove the minimum value among all the runs.
   * @return the minimum triple removed
   * @exception IOException from lower layers
   * @exception SortException something went wrong in the lower layer. 
   */
  private Triple delete_min() 
    throws IOException, 
	   SortException,
	   Exception
  {
    pnode cur_node;                // needs pq_defs.java  
    Triple new_triple, old_triple;  

    cur_node = Q.deq();
    old_triple = cur_node.triple;
    /*
    System.out.print("Get ");
    old_triple.print(_in);
    */
    // we just removed one triple from one run, now we need to put another
    // triple of the same run into the queue
    if (i_buf[cur_node.run_num].empty() != true) { 
      // run not exhausted 
      new_triple = new Triple(); // need triple.java??

     /* try {
	new_triple.setHdr(n_cols, _in, str_lens);
      }
      catch (Exception e) {
	throw new SortException(e, "Sort.java: setHdr() failed");
      }*/
      
      new_triple = i_buf[cur_node.run_num].Get(new_triple);  
      if (new_triple != null) {
	/*
	System.out.print(" fill in from run " + cur_node.run_num);
	new_triple.print(_in);
	*/
	cur_node.triple = new_triple;  // no copy needed -- I think Bingjie 4/22/98
	try {
	  Q.enq(cur_node);
	} catch (UnknowAttrType e) {
	  throw new SortException(e, "Sort.java: UnknowAttrType caught from Q.enq()");
	} catch (TripleUtilsException e) {
	  throw new SortException(e, "Sort.java: TripleUtilsException caught from Q.enq()");
	} 
      }
      else {
	throw new SortException("********** Wait a minute, I thought input is not empty ***************");
      }
      
    }

    // changed to return Triple instead of return char array ????
    return old_triple; 
  }
  
  /**
   * Set lastElem to be the minimum value of the appropriate type
   * @param lastElem the triple
   * @param sortFldType the sort field type
   * @exception IOException from lower layers
   * @exception UnknowAttrType attrSymbol or attrNull encountered
   */
  private void MIN_VAL(Triple lastElem, AttrType sortFldType) 
    throws IOException, 
	   FieldNumberOutOfBoundException,
	   UnknowAttrType {

    //    short[] s_size = new short[Triple.max_size]; // need Triple.java
    //    AttrType[] junk = new AttrType[1];
    //    junk[0] = new AttrType(sortFldType.attrType);
    char[] c = new char[1];
    c[0] = Character.MIN_VALUE; 
    String s = new String(c);
    //    short fld_no = 1;
    EID subjectId = lastElem.getSubjectId();
    PID predicateId = lastElem.getPredicateId();
    EID objectId = lastElem.getObjectId();
    
    switch (_sort_fld) {
    case 1:
    	(subjectId.pageNo).pid=Integer.MIN_VALUE;
    	lastElem.setSubjectId(subjectId); 
    	break;
    case 2:
		subjectId.slotNo = Integer.MIN_VALUE;
    	lastElem.setSubjectId(subjectId);
    	break;
	case 3:
		(predicateId.pageNo).pid=Integer.MIN_VALUE;
		break;
	case 4:
		predicateId.slotNo = Integer.MIN_VALUE;
    	lastElem.setPredicateId(predicateId);
    	break;
    case 5:
    	(objectId.pageNo).pid=Integer.MIN_VALUE;
	    lastElem.setObjectId(objectId); 
	    break;
    	case 6:
		lastElem.setSubjectId(objectId);
	    break;
	case 7:
		lastElem.setConfidence(Integer.MIN_VALUE);
		break;
	 default:
      // don't know how to handle attrSymbol, attrNull
      //System.err.println("error in sort.java");
      throw new UnknowAttrType("Sort.java: don't know how to handle attrSymbol, attrNull");
    }
   return;
  }

  /**
   * Set lastElem to be the maximum value of the appropriate type
   * @param lastElem the triple
   * @param sortFldType the sort field type
   * @exception IOException from lower layers
   * @exception UnknowAttrType attrSymbol or attrNull encountered
   */
  private void MAX_VAL(Triple lastElem, AttrType sortFldType) 
    throws IOException, 
	   FieldNumberOutOfBoundException,
	   UnknowAttrType {

    //    short[] s_size = new short[Triple.max_size]; // need Triple.java
    //    AttrType[] junk = new AttrType[1];
    //    junk[0] = new AttrType(sortFldType.attrType);
    char[] c = new char[1];
    c[0] = Character.MAX_VALUE; 
    String s = new String(c);
    EID subjectId = lastElem.getSubjectId();
    PID predicateId = lastElem.getPredicateId();
    EID objectId = lastElem.getObjectId();
   
    //    short fld_no = 1;
    
    switch (_sort_fld) {
    case 1:
    	(subjectId.pageNo).pid=Integer.MAX_VALUE;
    	lastElem.setSubjectId(subjectId); 
    	break;
    case 2:
		subjectId.slotNo = Integer.MAX_VALUE;
    	lastElem.setSubjectId(subjectId);
    	break;
	case 3:
		(predicateId.pageNo).pid=Integer.MAX_VALUE;
		break;
	case 4:
		predicateId.slotNo = Integer.MAX_VALUE;
    	lastElem.setPredicateId(predicateId);
    	break;
    case 5:
    	(objectId.pageNo).pid=Integer.MAX_VALUE;
	    lastElem.setObjectId(objectId); 
	    break;
    	case 6:
		lastElem.setSubjectId(objectId);
	    break;
	case 7:
		lastElem.setConfidence(Integer.MAX_VALUE);
		break;
	 default:
      // don't know how to handle attrSymbol, attrNull
      //System.err.println("error in sort.java");
      throw new UnknowAttrType("Sort.java: don't know how to handle attrSymbol, attrNull");
    } 
    return;
  }
  


  /**
   * Cleaning up, including releasing buffer pages from the buffer pool
   * and removing temporary files from the database.
   * @exception IOException from lower layers
   * @exception SortException something went wrong in the lower layer. 
   */
  public void close() throws SortException, IOException
  {
    // clean up
    if (!closeFlag) {
       
      try {
	_am.close();
      }
      catch (Exception e) {
	throw new SortException(e, "Sort.java: error in closing iterator.");
      }

      if (useBM) {
	try {
	  free_buffer_pages(_n_pages, bufs_pids);
	} 
	catch (Exception e) {
	  throw new SortException(e, "Sort.java: BUFmgr error");
	}
	for (int i=0; i<_n_pages; i++) bufs_pids[i].pid = INVALID_PAGE;
      }
      
      for (int i = 0; i<temp_files.length; i++) {
	if (temp_files[i] != null) {
	  try {
	    temp_files[i].deleteFile();
	  }
	  catch (Exception e) {
	    throw new SortException(e, "Sort.java: Heapfile error");
	  }
	  temp_files[i] = null; 
	}
      }
      closeFlag = true;
    } 
  } 

}


