...
/**
 * Class Triple heap file page. The design assumes that triples are kept
 * compacted when deletions are performed.
 */

public class THFPage extends Page implements ConstSlot, GlobalConst {
	...
	/**
	 * Constructor of class THFPage open a THFPage and make this THFpage point
	 * to the given page
	 * 
	 * @param page
	...
	/**
	 * Constructor of class THFPage open a existed Thfpage
	 * 
	 * @param apage
	...
	/**
	 * Constructor of class THFPage initialize a new page
	 * 
	 * @param pageNo
	 ...
	 *            the slot number
	 * @param length
	 *            length of triple the slot contains
	 * @param offset
	 *            offset of triple
	 * @exception IOException
	 *                I/O errors
	 ...
	 * 
	 * @param triple
	 *            a triple to be inserted
	 * @return TID of triple, null if sufficient space does not exist
	 * @exception IOException
	 *                I/O errors in C++ Status insertTriple(char *recPtr, int
	 *                tripLen, TID& tid)
	 */
	public TID insertTriple(byte[] triple) throws IOException {
		TID tid = new TID();

		int tripLen = triple.length;
		int spaceNeeded = tripLen + SIZE_OF_SLOT;
		...
		if (spaceNeeded > freeSpace) {
			...
			usedPtr = Convert.getShortValue(USED_PTR, data);
			usedPtr -= tripLen; // adjust usedPtr
			Convert.setShortValue(usedPtr, USED_PTR, data);

			// insert the slot info onto the data page
			setSlot(i, tripLen, usedPtr);
	...
	/**
	 * delete the triple with the specified tid
	 * 
	 * @param tid
	 *            the triple ID
	 ...
	 *                Invalid slot number
	 * @exception IOException
	 *                I/O errors in C++ Status deleteTriple(const TID& tid)
	 */
	public void deleteTriple(TID tid) throws IOException,
			InvalidSlotNumberException {
		int slotNo = tid.slotNo;
		short tripLen = getSlotLength(slotNo);
		slotCnt = Convert.getShortValue(SLOT_CNT, data);
		...
		if ((slotNo >= 0) && (slotNo < slotCnt) && (tripLen > 0)) {
			...
			// to the left of the triple being removed. (by the size of the
			// hole)
	...
	/**
	 * @return TID of first triple on page, null if page contains no triples.
	 * @exception IOException
	 *                I/O errors in C++ Status firstTriple(TID& firstTid)
	 * 
	 */
	public TID firstTriple() throws IOException {
		TID tid = new TID();
	...
	/**
	 * copies out triple with LID lid into triple pointer. <br>
	 * Status getTriple(TID tid, char *recPtr, int& tripLen)
	 * 
	 * @param tid
	 *            the triple ID
	 ...
	 * @see Triple
	 */
	public Triple getTriple(TID tid) throws IOException,
			InvalidSlotNumberException {
		short tripLen;
		short offset;
		byte[] tripleArray;
	...
	/**
	 * returns a triple in a byte array[pageSize] with given TID tid. <br>
	 * in C++ Status returnTriple(TID tid, char*& recPtr, int& tripLen)
	 * 
	 * @param tid
	 *            the triple ID
	 ...
	 * @see Triple
	 */
	public Triple returnTriple(TID tid) throws IOException,
			InvalidSlotNumberException {
		short tripLen;
		short offset;
		byte[] tripleArray;
	 ...
	 * 
	 * @return true if the THFPage is has no triples in it, false otherwise
	 * @exception IOException
	 *                I/O errors
	 */
	...
	/**
	 * Compacts the slot directory on an THFPage. WARNING -- this will probably
	 * lead to a change in the TIDs of triples on the page. You CAN'T DO THIS on
	 * most kinds of pages.
	 * 
	 * @exception IOException
	 *                I/O errors
	...
	protected void compact_slot_dir() throws IOException {
		...
		boolean move = false; // Move a triple? -- initially false
		short length;
		short offset;
